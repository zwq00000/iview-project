...
*/
const GetWarehouses = function(searchStr,pageSize,page){
...
* @returns axios.get Promission
*/
const GetWarehouse = function(whCode){
    let path = `/api/Warehouse/${whCode}`;
    let params = {};
 ...
 */
const GetWhCodes = function(depCode){
    if(!depCode){
        depCode = '002';
    }
    let path=`/api/Warehouse/whcodes/${depCode}`;
    return fetch.get(path);
};

export default{ 
    GetWarehouses,GetWarehouse,GetWhCodes
};
